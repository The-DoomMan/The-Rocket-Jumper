...
namespace RocketJumper
{
    class RocketJumper : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (MonoSingleton<InputManager>.Instance.InputSource.Fire1.IsPressed && RJAnimator && fireCD <= 0 && !detonating)
            {
                ...
                Rocket.AddComponent<RemoveOnTime>();
                Rocket.AddComponent<ShaderSwapper>();
                Rocket.GetComponent<RemoveOnTime>().time = 13f;
                Rocket.GetComponent<RemoveOnTime>().useAudioLength = false;
                Rocket.GetComponent<RemoveOnTime>().randomizer = 0f;
